...
function validateMetadata(metadata) {
  ...
  if (metadata.p) {
    ...
    // NFT requires either content.primary (v2 explicit form) or the legacy
    // v1 'main' shorthand. Whitepaper §21.4-21.5 covers both forms.
    if (protocols.includes(GlyphProtocol.GLYPH_NFT)) {
      var hasPrimary = metadata.content && metadata.content.primary
      var hasMain = metadata.main != null
      if (!hasPrimary && !hasMain) {
        errors.push('NFT requires content.primary or main')
      }
    }
...
function validateContentFile(file, path, errors) {
  ...
  if (!file.hash || !file.hash.algo || !file.hash.hex) {
    ...
    // Validate hex string structure: must be even-length hex, length must
    // match the algorithm's digest size.
    const hex = file.hash.hex
    if (typeof hex !== 'string' || !/^[0-9a-fA-F]+$/.test(hex) || hex.length % 2 !== 0) {
      errors.push(`${path}.hash.hex must be an even-length hex string`)
    } else {
      const expectedByteLengths = { sha256: 32, sha512: 64, blake3: 32, k12: 32 }
      const algo = String(file.hash.algo).toLowerCase()
      const expected = expectedByteLengths[algo]
      if (expected !== undefined && hex.length !== expected * 2) {
        errors.push(`${path}.hash.hex length ${hex.length / 2} bytes does not match ${algo} digest size ${expected}`)
      }
    }
  }
}
...
function validateRoyalty(royalty, errors) {
  ...
  if (royalty.splits && Array.isArray(royalty.splits)) {
    if (royalty.splits.length === 0) {
      errors.push('Royalty splits must be omitted or non-empty')
    }
    let totalBps = 0
...
/**
 * Check if protocols include Fungible Token
 * @param {number[]} protocols
 * @returns {boolean}
 */
function isFungible(protocols) {
  return Array.isArray(protocols) && protocols.includes(GlyphProtocol.GLYPH_FT)
}

/**
 * Check if protocols include dMint
 * @param {number[]} protocols
 * @returns {boolean}
 */
function isDmint(protocols) {
  return Array.isArray(protocols) && protocols.includes(GlyphProtocol.GLYPH_DMINT)
}

/**
 * Check if protocols include Mutable
 * @param {number[]} protocols
 * @returns {boolean}
 */
function isMutable(protocols) {
  return Array.isArray(protocols) && protocols.includes(GlyphProtocol.GLYPH_MUT)
}

/**
 * Check if protocols include Container
 * @param {number[]} protocols
 * @returns {boolean}
 */
function isContainer(protocols) {
  return Array.isArray(protocols) && protocols.includes(GlyphProtocol.GLYPH_CONTAINER)
}

/**
 * Check if protocols include Encrypted
 * @param {number[]} protocols
 * @returns {boolean}
 */
function isEncrypted(protocols) {
  return Array.isArray(protocols) && protocols.includes(GlyphProtocol.GLYPH_ENCRYPTED)
}
