...
Message.prototype._verify = function _verify (publicKey, signature) {
  ...
  // Reject malleable (non-low-S) signatures so verify(m, s) and verify(m, n-s)
  // do not both succeed. Matches the BIP62 / consensus invariant the script
  // interpreter enforces via SCRIPT_VERIFY_LOW_S.
  if (!signature.hasLowS()) {
    this.error = 'The signature was malleable (high S)'
    return false
  }
  var hash = this.magicHash()
