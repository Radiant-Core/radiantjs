...
/**
 * Derive the Radiant default external-chain HD private key for an account.
 *
 * Uses the Radiant SLIP-0044 coin type (512) per the v3.0.0 ecosystem standard:
 *
 *   m / 44' / 512' / account' / 0 / addressIndex
 *
 * @param {String=} passphrase BIP39 passphrase, defaults to ''.
 * @param {Number=} addressIndex Final non-hardened index, defaults to 0.
 * @param {Number=} account Hardened account index, defaults to 0.
 * @param {Network|String|number=} network Network for the derived xprv.
 * @returns {HDPrivateKey}
 */
Mnemonic.prototype.toHDPrivateKeyAtDefaultPath = function (passphrase, addressIndex, account, network) {
  if (addressIndex == null) addressIndex = 0
  if (account == null) account = 0
  var master = this.toHDPrivateKey(passphrase, network)
  return master.deriveChild("m/44'/" + Mnemonic.RADIANT_COIN_TYPE + "'/" + account + "'/0/" + addressIndex)
}
...
// SLIP-0044 registered coin type for Radiant (RXD); standardised in the
// v3.0.0 ecosystem release. Wallets should derive at m/44'/512'/.../.../...
Mnemonic.RADIANT_COIN_TYPE = 512
Mnemonic.RADIANT_DEFAULT_PATH = "m/44'/512'/0'/0"
