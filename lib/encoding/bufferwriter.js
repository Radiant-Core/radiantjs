...
class BufferWriter {
  ...
  writeVarintBN (bn) {
    // Branch on bit length first; calling bn.toNumber() up-front throws on
    // BN values whose magnitude exceeds 2^53 - which are precisely the
    // values that need this BN-aware path.
    if (bn.bitLength() > 32) {
      var bw = new BufferWriter()
      bw.writeUInt8(255)
      bw.writeUInt64LEBN(bn)
      this.write(bw.toBuffer())
      return this
    }
    var n = bn.toNumber()
    if (n < 253) {
