...
HDPrivateKey._getDerivationIndexes = function (path) {
  ...
  var indexes = steps.slice(1).map(function (step) {
    ...
    // Strict: each index must be a non-negative decimal integer. Rejects
    // "1e5", "0x10", " 3 ", "" and similar - +step would silently coerce.
    if (!/^[0-9]+$/.test(step)) {
      return NaN
    }
    var index = +step
...
// Self-contained invariant (was `assert(...)`): this runs at module load, and
// under some bundler emit orders (e.g. Rollup code-splitting radiantjs into a
// lazy chunk) the module-scope `assert` import is not yet initialized here —
// crashing every consumer of the chunk with "TypeError: e is not a function".
// A plain conditional throw checks the same invariant with no load-order
// dependency.
