...
describe('Interpreter', function () {
  ...
  // Bitcoin Core script_tests vectors that are known to differ in Radiant due to:
  // - 64-bit integer support (Radiant accepts >32-bit integers that Bitcoin rejects)
  ...
  describe('bitcoind script evaluation fixtures', function () {
    var testAllFixtures = function (set) {
      ...
      set.forEach(function (vector) {
        ...
        if (radiantSkipVectors.has(c)) {
          it.skip(txt + ' [Radiant-specific difference]', function () {})
        } else {
